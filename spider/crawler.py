...
class Crawler():
	...
	def run(self):
		# print (sitemapConfig.xml_header, file=self.output_file)
		print >> self.output_file,sitemapConfig.xml_header
	...
	def __crawling(self):
		...
		request = urllib2.Request(crawling, headers={"User-Agent":sitemapConfig.crawler_user_agent})#Request(crawling, headers={"User-Agent":sitemapConfig.crawler_user_agent})
		...
		print >> self.output_file,"<url><loc>"+url.geturl()+"</loc><lastmod>"+date.strftime('%Y-%m-%dT%H:%M:%S+00:00')+"</lastmod></url>"
		if self.output_file:
	...
	def checkRobots(self):
		if self.domain[len(self.domain)-1] != "/":
			self.domain += "/"
		request = Request(self.domain+"robots.txt", headers={"User-Agent":sitemapConfig.crawler_user_agent})
